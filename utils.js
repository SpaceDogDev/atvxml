	...
	var server = http.createServer(function(request, response) {
      ...
      response.write('hello');
        ...
        function(callback){
          ...
          response.write(AppleXMLContent);
...
/*		
		for (var i = 0; i < length; i++) {
		    if (pathname == directories[i].uri+".xml") {

      		console.log("pathname:"+pathname);
      		//remove the .xml from the string to run through the plex-api
      		var plexPath = pathname;
          plexPath = plexPath.substring(0, plexPath.length - 4)
          console.log("plexPath:"+plexPath);

          queryPlex(plexPath);
		      directories = xml.directory;
		      length = xml.length;
		      
		      response.writeHead(200, {'Content-Type': 'text/xml'});
          response.write(AppleIndexHeader);
          response.write('<moviePoster id="shelf_item_1" accessibilityLabel="'+directories[i].attributes.title+'" featured="true" onSelect="atv.loadURL(\'http://trailers.apple.com'+directories[i].uri+'.xml\');" onPlay="atv.loadURL(\'http://trailers.apple.com'+directories[i].uri+'.xml\');"><image>'+directories[i].attributes.thumb+'</image><defaultImage>'+directories[i].attributes.thumb+'</defaultImage><title>'+directories[i].attributes.title+'</title></moviePoster>');
          response.write(AppleIndexFooter);
		      response.end();
		  }
		}
...
function queryPlex(key, callback) {
  console.log("key:"+key);
  ...
  client.query(key, function (error, result) {
      if (error) {
        throw new Error("Could not connect to server");
      			...
      			//get the directory xml
		  directories = result.directory;
		  console.log(directories);
      length = directories.length;
      console.log(length);
      for (var i = 0; i < length; i++) {
        AppleXMLContent = '<moviePoster id="shelf_item_1" accessibilityLabel="'+directories[i].attributes.title+'" featured="true" onSelect="atv.loadURL(\'http://trailers.apple.com'+directories[i].uri+'.xml\');" onPlay="atv.loadURL(\'http://trailers.apple.com'+directories[i].uri+'.xml\');"><image>'+directories[i].attributes.thumb+'</image><defaultImage>'+directories[i].attributes.thumb+'</defaultImage><title>'+directories[i].attributes.title+'</title></moviePoster>';
        }
    });
}
